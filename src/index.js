...
const dispatcherFn = require('./dispatcher')
...
function forEachAction (state, filter, iterator) {
  ...
  return _.reduce(state.resources, function (acc, resource, resourceName) {
    const results = _.map(resource.actions, function (action, actionName) {
...
function forEachResource (state, filter, iterator) {
  ...
  const results = _.map(state.resources, function (resource, resourceName) {
    if (filter(resource)) {
...
function handle (state, envelope) {
  ...
  if (handler) {
    ...
    return Promise.resolve({
      status: 404,
      data: `No handler found for ${envelope.resource} - ${envelope.action}`
    })
  }
}

function initialize (supplied) {
  const config = normalizeConfig(supplied)
  const fount = config.fount
  const metronic = config.metronic(config.metrics)
...
function initializeDispatcher (state) {
  const dispatcher = dispatcherFn(state)
...
function loadExtensions (state, type) {
  ...
  const container = state.fount(type)
  ...
  return state.modlo
    ...
    .then(function (result) {
      const promises = _.map(result.loaded, function (extension) {
        return container
          .resolve(extension)
          .then(function (result) {
